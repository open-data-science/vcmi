...
#endif
{
    ...
    ("disable-video,dv", "disable video player")
    ("nointro,i", "skips intro movies")
    ("donotstartserver,d","do not attempt to start server and just connect to it instead server")
    ("serverport", po::value<si64>(), "override port specified in config file")
    ("saveprefix", po::value<std::string>(), "prefix for auto save files")
    ("savefrequency", po::value<si64>(), "limit auto save creation to each N days")
    ("host,ho", po::value<std::string>(), "create host game, string argument is string array separated by comma of param: [filename=string, isSave=boolean")
    ...
    *console->cb = processCommand;
    ...
    {
        logGlobal->info("Initialization of VCMI multiplayer host.", total.getDiff());
        std::string stringArraySeparatedByComma = vm["host"].as<std::string>();
        logGlobal->info("Multiplayer host params: %s", stringArraySeparatedByComma);

        // TODO: refactor to outer function
        std::string s = stringArraySeparatedByComma;
        std::string delimiter = ",";
        std::string filename = s.substr(0, s.find(delimiter));
        std::string isSave = s.substr(s.find(delimiter) + 1,1);

        logGlobal->info("Parsed filename: %s", filename);
        logGlobal->info("Parsed isSave: %s", isSave);

        // TODO: refactore to outer function
        bool boolIsSave;
        if (isSave == "0")
            boolIsSave = false;
        else if (isSave == "1")
        ...
        boost::thread(&CServerHandler::debugStartHost, CSH, filename, boolIsSave);
    }
    else if (vm.count("join"))
    {
        logGlobal->info("Initialization of VCMI multiplayer join.", total.getDiff());
        std::string stringArraySeparatedByComma = vm["join"].as<std::string>();
        logGlobal->info("Multiplayer join params: %s", stringArraySeparatedByComma);

        // TODO: refactor to outer function
        std::string s = stringArraySeparatedByComma;
        std::string delimiter = ":";
        std::string server = s.substr(0, s.find(delimiter));
        std::string port = s.substr(s.find(delimiter) + 1,4);

        logGlobal->info("Parsed server: %s", server);
        logGlobal->info("Parsed port: %s", port);

        // TODO: refactore to outer function
        boost::thread(&CServerHandler::debugStartJoin, CSH, filename, std::stoi(port));
    }
    else
    {
