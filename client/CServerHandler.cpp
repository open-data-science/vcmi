...
void CServerHandler::justConnectToServer(const std::string & addr, const ui16 port)
{
    ...
    if(state == EClientState::CONNECTION_CANCELLED)
        ...
        logNetwork->info("Connection thread start!");
...
{
    logGlobal->info("start client connecting");
    ...
    logGlobal->info("end client connecting");
}
...
void CServerHandler::sendHostGame(bool allowOnlyAI) const
{
    verifyStateBeforeStart(allowOnlyAI ? true : settings["session"]["onlyai"].Bool());
    LobbyStartGame lsg;
    if(client)
...
void CServerHandler::debugStartJoin(std::string server, int port)
{
    logGlobal->info("Starting debug join with server: %s, port: %d", server, port);

    logGlobal->info("resetStateForLobby.");

    resetStateForLobby(StartInfo::NEW_GAME);


    logGlobal->trace("screenType::newGame.");
    screenType = ESelectionScreen::newGame;

    boost::this_thread::sleep(boost::posix_time::milliseconds(3000));
    logGlobal->info("Try connect to server.");
    justConnectToServer("127.0.0.1", 3030);
    logGlobal->info("Connected to server.");

    setPlayer(myFirstColor());
    logGlobal->info("Player setted.");

    boost::this_thread::sleep(boost::posix_time::milliseconds(1000));
    logGlobal->info("Try start game...");

    // sendStartGame();
}

void CServerHandler::debugStartHost(std::string filename, bool save)
{
    logGlobal->info("Starting debug test with file: %s", filename);

    auto mapInfo = std::make_shared<CMapInfo>();
    logGlobal->info("Map info inited.");

    if(save)
    {
        logGlobal->info("resetStateFor lobby: LOAD_GAME");
    ...
    {
        logGlobal->info("resetStateFor lobby: NEW_GAME");
        resetStateForLobby(StartInfo::NEW_GAME);
    ...
    if(settings["session"]["donotstartserver"].Bool()) {
        logGlobal->info("Starting connection to server.");
        justConnectToServer("127.0.0.1", 3030);
    }
    else {
        logGlobal->info("Starting Local server and connection to it.");
        startLocalServerAndConnect();
    }

    setMapInfo(mapInfo);
    logGlobal->trace("Map info setted.");

    setPlayer(myFirstColor());
    logGlobal->trace("Player setted.");

    boost::this_thread::sleep(boost::posix_time::milliseconds(1000));
    logGlobal->trace("Try start game...");
...
void CServerHandler::threadHandleConnection()
{
    ...
    logGlobal->info("enterLobbyConnectionMode ended.");
